...
velocities['max'] = velocities.max(axis=1, skipna=True)
velocities['min'] = velocities.min(axis=1, skipna=True)
...
length = transposed.iloc[1:]  # get rid of redundant line
length = length.loc[length.index.str.startswith('posX')]
length = length.transpose()
length['max'] = length.max(axis=1, skipna=True)
...
df['max_velX'] = velocities['max']
df['min_velX'] = velocities['min']
df['upwards'] = df['avg_velZ'] > 0
df['max_height']= heights['max']
df['max_length']= length['max']
...
# df = df.rename(columns={'0': 'max_group_height'})
# print(df.head())
...
# df['height_param']=df.apply(lambda x: log(x['height_p']))
# CHANGE NAME
